    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path,
                                        image_name=image_name, width=image.size[0], height=image.size[1], mask=mask,
                                        bounds=bounds))
    return cam_infos


def readNerfSyntheticInfo(path, white_background, eval, rand_pcd, llffhold=8, n_views=8, extension=".png"):
    ...
    print('test', [info.image_path for info in test_cam_infos])
